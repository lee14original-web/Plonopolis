-- ============================================================
-- AKTUALIZACJA: game_harvest_plot — losowa jakość zwykłych nasion
-- Szanse: 10% popsuta, 85% zwykła, 4% epicka, 1% legendarna
...
begin
  ...
  if v_actual_planted_q = 'rotten' then
    ...
    -- Zwykłe nasiono: losuj jakość plonu
    -- 10% popsuta | 85% zwykła | 4% epicka | 1% legendarna
    v_q_roll := random();
    if v_q_roll < 0.10 then
      v_target_quality := 'rotten';
    elsif v_q_roll < 0.95 then
      v_target_quality := 'good';
    elsif v_q_roll < 0.99 then
  ...
  v_gained_legendary := case when v_target_quality = 'legendary' then v_crop.yield_amount else 0 end;
  ...
  if p_zrecznosc > 0 and random() < v_zrecznosc_chance then
    ...
    elsif v_target_quality = 'legendary' then
      v_gained_legendary := v_gained_legendary + v_crop.yield_amount;
    else
