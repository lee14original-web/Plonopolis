-- ============================================================
-- AKTUALIZACJA: game_harvest_plot — legendarny system 3 opcji
-- Wklej do Supabase SQL Editor i wykonaj
-- ============================================================

-- Jeśli istnieje stara wersja z p_exp_mult — usuń ją najpierw:
...
CREATE OR REPLACE FUNCTION public.game_harvest_plot(
  ...
  p_exp_mult_override    integer DEFAULT 0   -- 0 = wylicz z jakości; >0 = dokładny mnożnik (EXP opt. legendarny)
)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
begin
  ...
  -- ── Normalny zbiór (bazowa ilość; SQL dodaje do klucza crop.id) ──────────
  ...
  -- ── EXP — nowy system mnożników ──────────────────────────────────────────
  -- p_exp_mult_override > 0 → dokładny mnożnik (używany przy legendarnm opt. EXP: 15–30x)
  -- p_planted_quality = 'rotten'  → 0 EXP
  -- p_planted_quality = 'epic'    → losowo 3–6x
  -- pozostałe (good / legendary opt. 1 i 2) → 1x (bazowy)
  ...
  ELSIF p_planted_quality = 'epic' THEN
    v_exp_mult := 3 + floor(random() * 4);  -- 3–6x losowo
  ELSE
    v_exp_mult := 1;
  END IF;
