-- ============================================================
-- PATCH: game_harvest_plot — per-item quality rolls dla wszystkich typów nasion
--
-- Zmiany vs poprzedniej wersji:
--   1. ZWYKŁE nasiono: każda sztuka losuje jakość osobno (nie jeden rzut na całą partię)
--      yield_amount <= 2 → 1–3 szt.; > 2 → 2–6 szt.
--   2. EPICKIE nasiono: yield_amount > 2 → 20–44 szt. (było 14–30)
--   3. LEGENDARNE nasiono: per-item rolls zamiast gwarantowanego good+epic;
--      yield_amount <= 2 → 20–60 szt.; > 2 → 40–120 szt.
--   4. EXP zależy wyłącznie od jakości ZASADZONEGO nasiona (nie wylosowanego plonu):
--      zwykłe → ×1 | epickie → ×3–6 | legendarne → ×10–20 | popsute zasadzone → ×0
--      Popsuta sztuka plonu NIE zeruje EXP całego pola.
--   5. Zręczność: podwaja finalne ilości wszystkich jakości.
--   6. Progi Szczęścia, kompost, extra_harvest, bonus_drop — bez zmian.
--
