-- ============================================================
-- PATCH: game_harvest_plot — naprawa mnożnika EXP dla legendarnych
-- Problem: yield_amount > 2 dostawał x12-25 zamiast x10-20.
-- Fix: zawsze x10-20, bez rozgałęzienia na yield_amount.
-- Marchewka (exp_reward=6): epic → 18-36 EXP, legendary → 60-120 EXP.
...
begin
  ...
  -- ── EXP — mnożnik bazowy ─────────────────────────────────────────────────
  -- zwykła:    x1
  -- epicka:    losowo x3–6  (3 + floor(random() * 4))
  -- legendarna: losowo x10–20 (10 + floor(random() * 11))  ← POPRAWKA: jednolity zakres bez rozgałęzienia na yield_amount
  -- popsuta:   x0
  -- Bonusy % stosowane po mnożniku jakości.
  ...
  ELSIF v_actual_planted_q = 'rotten' OR v_target_quality = 'rotten' THEN
    v_exp_mult := 0;
  ELSIF v_actual_planted_q = 'legendary' THEN
    -- POPRAWKA: zawsze 10–20, niezależnie od yield_amount
  ...
  -- Bonusy % po mnożniku jakości: base × mult × (1 + bonus%)
