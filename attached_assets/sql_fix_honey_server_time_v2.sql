-- =====================================================
-- FIX v2: collect_honey zwraca server_now_ms w każdej odpowiedzi
-- + funkcja pomocnicza get_server_time_ms dla kalibracji zegara
--
-- Problem: klient liczy elapsed od lokalnego Date.now(),
-- który może wyprzedzać serwer o kilkanaście minut.
-- Fix: klient dostaje server_now_ms z każdego wywołania collect_honey
-- i kalibruje swój zegar lokalny (hiveNow) do czasu serwera.
-- =====================================================

-- Pomocnicza funkcja — zwraca aktualny czas serwera w ms (epoch)
...
-- collect_honey z server_now_ms w każdej odpowiedzi
...
BEGIN
  ...
  IF auth.uid() IS DISTINCT FROM p_user_id THEN
    RETURN jsonb_build_object('ok', false, 'error', 'unauthorized', 'server_now_ms', v_now_ms);
  END IF;
  ...
  IF v_hive IS NULL THEN
    RETURN jsonb_build_object('ok', false, 'error', 'no_hive', 'server_now_ms', v_now_ms);
  END IF;
  ...
  IF v_honey_avail <= 0   THEN RETURN jsonb_build_object('ok', false, 'error', 'no_honey',  'server_now_ms', v_now_ms, 'honey_start', v_honey_start); END IF;
  IF v_empty_jars <= 0    THEN RETURN jsonb_build_object('ok', false, 'error', 'no_jars',   'server_now_ms', v_now_ms); END IF;
  IF v_suit <= 0          THEN RETURN jsonb_build_object('ok', false, 'error', 'no_suit',   'server_now_ms', v_now_ms); END IF;
