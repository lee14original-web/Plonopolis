-- =====================================================
-- SMART RESET v2: Timer przy częściowym odbiorze
-- Uruchom RAZ w Supabase SQL Editor (zastępuje v1)
--
-- Zasada:
--   Magazyn PEŁNY  → reset timera do v_now_ms
--   Magazyn NIEPEŁNY → zachowaj postęp bieżącego cyklu
--     new_start = v_now_ms - (elapsed % cycle_ms)
--
...
BEGIN
  ...
  -- SMART RESET: pełny → reset do now; niepełny → zachowaj postęp cyklu
  IF v_cycles >= v_storage_cap THEN
    v_new_pstart := v_now_ms;
  ELSE
    v_new_pstart := v_now_ms - (v_elapsed % v_tree.growth_time_ms);
  END IF;
...
BEGIN
  ...
  -- SMART RESET: pełny → reset do now; niepełny → zachowaj postęp cyklu
  IF v_cycles >= v_animal.storage_max THEN
    v_new_pstart := v_now_ms;
  ELSE
    v_new_pstart := v_now_ms - (v_elapsed % v_animal.prod_ms);
  END IF;
...
BEGIN
  ...
  -- SMART RESET: magazyn pełny → reset do now; niepełny → zachowaj postęp cyklu
  IF v_honey_avail_raw >= v_max_honey THEN
    v_new_honey_start := v_now_ms;
  ELSE
    v_new_honey_start := v_now_ms - (v_elapsed_ms % v_ms_per_pt);
  END IF;
