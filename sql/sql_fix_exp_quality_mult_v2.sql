-- ════════════════════════════════════════════════════════════════════════
-- PATCH: game_harvest_plot — naprawa mnożników EXP per jakość zasadzonego nasiona
--
-- Zmiany (tylko blok EXP, reszta bez zmian):
--   PRZED:  legendary ×2 stały, epic ×1.5 stały, cap LEAST(..., 4.0)
--   PO:     legendary ×10–20 losowy, epic ×3–6 losowy, cap usunięty
--           (zostaje tylko soft-cap: legendary ≤ exp_reward × 50)
--
-- Marchewka (exp_reward=6, bez bonusów):
--   good       →   6 × 1        =  6 EXP
--   epic       →   6 × (3–6)    = 18–36 EXP
--   legendary  →   6 × (10–20)  = 60–120 EXP
--   rotten     →   0 EXP
--
-- Bonusy % EXP naliczane po mnożniku jakości (bez zmian w logice bonusów).
-- Dropy, ilości plonów, podlewanie, sadzenie, DB schema — bez zmian.
--
...
begin
  ...
  -- ── EXP — mnożnik per jakość zasadzonego nasiona ─────────────────────────
  -- ZMIANA vs poprzedni patch (sql_fix_xp_to_next_level_v1.sql):
  --   legendary: ×2 stały   → ×10–20 losowy
  --   epic:      ×1.5 stały → ×3–6 losowy
  --   usunięto:  LEAST(..., 4.0) — cap blokował epic/legendary przy bonusach
  --   dodano:    soft-cap legendary: v_exp_gained ≤ exp_reward × 50
  ...
  ELSIF v_actual_planted_q = 'rotten' OR v_target_quality = 'rotten' THEN
    v_exp_mult := 0;
  ELSIF v_actual_planted_q = 'legendary' THEN
    v_legendary_exp_mult := 10 + floor(random() * 11)::integer;  -- losowo 10–20
    v_exp_mult := v_legendary_exp_mult;
  ELSIF v_actual_planted_q = 'epic' THEN
    v_exp_mult := 3 + floor(random() * 4);                        -- losowo 3–6
  ELSE
    v_exp_mult := 1;                                               -- good: ×1
  END IF;

  -- Bonusy % EXP po mnożniku jakości; cap bonusów 500% żeby nie overflowować
  v_exp_gained := ROUND(v_crop.exp_reward * v_exp_mult * (1.0 + LEAST(p_exp_bonus_pct, 500.0) / 100.0));
  -- Soft-cap dla legendarnych: ≤ exp_reward × 50
