-- ============================================================
-- POPRAWKA KRYTYCZNA: game_harvest_plot — atomiczny zbiór
-- Wklej do Supabase SQL Editor i wykonaj
-- ============================================================
--
-- NAPRAWIA BUG:
--   - Stary SQL dodawał plon pod kluczem `crop.id` (np. "carrot")
--     zamiast pod kluczem z jakością (np. "carrot_good").
--   - W połączeniu z asynchronicznym zbiorem 6 pól naraz +
--     klientem który nadpisywał DB pełnym obiektem inwentarza,
--     powstawała race condition akumulująca plony (np. 80 zamiast 30).
--
-- ZMIANY:
--   1. Plon zapisywany pod prawidłowym kluczem `crop_id || '_good'`.
--   2. Bonus z kompostu (yield), % extra harvest i % bonus drop
--      aplikowane atomicznie po stronie SQL — koniec race condition.
--   3. Funkcja zwraca explicit liczby `gained_good`, `gained_epic`,
--      `gained_rotten` — klient nie musi już liczyć diff'a względem
--      snapshot'a stanu (który był niepewny przy wielu zbiorach naraz).
--   4. ANTI-EXPLOIT: jakość zasadzonego nasiona (`plantedQuality`) i bonus
--      z kompostu (`compostBonus.value`, max 3) są odczytywane z DB
--      (`v_plot`), a nie z parametrów RPC — klient nie może ich zawyżyć.
--   5. Compost yield zawsze trafia do `_good` (jak stary klient) —
--      działa też przy zbiorze epic/rotten.
--
-- WSTECZNA KOMPATYBILNOŚĆ:
--   - Stare wpisy w seed_inventory pod kluczem "carrot" (bez sufiksu)
--     są MIGROWANE do "carrot_good" przy starcie funkcji.
--
-- LEGENDARNE PLONY:
--   - SQL traktuje legendarny jak "good" (p_planted_quality='good').
--   - Klient nadal sam losuje opcję legendarnego (zwykłe / epickie / EXP)
--     i dodaje ręcznie do inwentarza po stronie klienta.
--
-- Aby zainstalować: wklej cały plik do Supabase SQL Editor i wykonaj.
-- ============================================================
...
CREATE OR REPLACE FUNCTION public.game_harvest_plot(
  ...
  p_compost_yield_extra  integer DEFAULT 0,   -- bonus z kompostu Urodzaju (1/2/3)
  p_extra_harvest_pct    numeric DEFAULT 0,   -- % z eq na +1 sztukę za każdą GOOD
  p_bonus_drop_pct       numeric DEFAULT 0    -- % z eq na upgrade GOOD → EPIC
)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
begin
  ...
  -- Klucze z jakością (NOWY format)
  v_good_key   := v_crop.id || '_good';
  v_epic_key   := v_crop.id || '_epic';
  v_rotten_key := v_crop.id || '_rotten';
  v_legacy_key := v_crop.id;  -- stary klucz bez sufiksu

  -- ── MIGRACJA: stary klucz bez jakości → "_good" ──────────────────────────
  -- Jeśli w inventory jest "carrot" (bez sufiksu) — przenieś do "carrot_good"
  ...
  if v_legacy_amount > 0 then
    ...
    v_seed_inventory := v_seed_inventory - v_legacy_key;  -- usuń stary klucz
  end if;

  -- ── Jakość ZASADZONEGO nasiona — odczyt z DB (anti-exploit) ──────────────
  -- SQL nie ufa parametrowi p_planted_quality (klient mógłby kłamać).
  -- Czytamy z v_plot.plantedQuality (zapisane przy sadzeniu po stronie SQL).
  ...
  -- ── Wybór docelowej jakości plonu (rotten / good / epic) ─────────────────
  if v_actual_planted_q = 'rotten' then
    v_target_quality := 'rotten';
  elsif v_actual_planted_q = 'epic' then
    v_target_quality := 'epic';
  else
    v_target_quality := 'good';  -- 'good' i 'legendary' (legendarny rozdziela klient)
  end if;
  ...
  -- ── Zręczność: szansa na podwójny zbiór (kolejne yield_amount sztuk) ─────
  ...
  -- Odczyt z DB (v_plot.compostBonus) — anti-exploit; klient mógłby kłamać.
  -- Compost yield zawsze trafia do _good (jak stary klient — niezależnie od quality).
  -- Działa też przy zbiorze epic/rotten — daje +N _good obok bazowych _epic/_rotten.
  ...
  -- (działa tylko na zwykłych zbiorach, jak w klieńcie)
  ...
  -- ── Zapis do seed_inventory: WSZYSTKO ATOMICZNIE pod kluczami z jakością ─
  ...
  -- p_exp_mult_override > 0 → dokładny mnożnik (legendarna opcja EXP: 15–30x)
  -- p_exp_mult_override = -1 → wymuś 0 EXP (legendarna opcja 0/1)
  -- p_planted_quality = 'rotten' → 0 EXP
  -- p_planted_quality = 'epic'   → losowo 3–6x
  -- pozostałe (good / legendary) → 1x
  ...
  -- Zwróć profil + szczegółowe info dla klienta
