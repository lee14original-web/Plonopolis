-- ============================================================
-- PATCH: game_harvest_plot — legendarny drop server-side
-- Problem: klient obliczał _legGood/_legEpic lokalnie i nadpisywał
--          pełne inventory przez profiles.update. Przy równoległym
--          "Zbierz wszystko" race condition: ostatni zapis wygrywał,
--          ale popup sumował wszystkie pola → popup > plecak.
-- Fix: SQL sam oblicza legendarne plony + EXP mult atomicznie
--      i zwraca gained_good / gained_epic / legendary_exp_mult.
...
begin
  ...
  -- Używane dla zwykłych nasion i per-item rollów dla epickich
  ...
  if v_actual_planted_q = 'rotten' then
    ...
    -- ── LEGENDARNY: SQL liczy drop atomicznie (eliminuje race condition) ────
    -- lvl 1-6 (yield_amount<=2): 20-60 good + 5-12 epic
    -- lvl 7+  (yield_amount>2):  30-80 good + 8-18 epic
    ...
    -- ── EPICKIE: losowy yield, każda sztuka losuje jakość osobno ────────────
  ...
  -- Nie dotyczy legendarnych (p_extra_harvest_pct=0 dla legendary po stronie klienta)
  ...
  -- Nie dotyczy legendarnych (p_bonus_drop_pct=0 dla legendary po stronie klienta)
  ...
  -- p_exp_mult_override > 0  → dokładny mnożnik (legacy / override)
  -- legendary                → SQL generuje losowy mnożnik
  ...
  ELSIF v_actual_planted_q = 'rotten' OR v_target_quality = 'rotten' THEN
    v_exp_mult := 0;
  ELSIF v_actual_planted_q = 'legendary' THEN
    -- SQL generuje mnożnik EXP dla legendarnych (eliminuje klient-side random)
    if v_crop.yield_amount <= 2 then
