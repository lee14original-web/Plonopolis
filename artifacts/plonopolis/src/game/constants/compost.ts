...
export const COMPOST_DEFS: Record<CompostType, { id: string; name: string; icon: string; desc: string; effectLabel: string; bonusValues: number[]; bonusLabel: (v: number) => string; tierName: (v: number) => string }> = {
  ...
  guide:  { id: "guide_compost",  name: "Kompost Przewodnika",  icon: "🌟", desc: "Specjalny kompost dla początkujących. Skraca czas wzrostu uprawy o 65% (globalny cap). Nie można go sprzedać na targu.", effectLabel: "🌟 −65% czasu wzrostu", bonusValues: [65], bonusLabel: () => `−65% czasu wzrostu`, tierName: () => "Kompost Przewodnika" },
};
