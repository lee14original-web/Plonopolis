...
import { barnFmtMs, plItem } from "../../utils/barn";
...
export function BarnModal({ displayLevel, displayMoney, barnState, effectiveStats, barnNow, onClose, onBuySlot, onCollect, onCollectAll }: BarnModalProps) {
  ...
  const totalStorage = ANIMALS.reduce((s, a) => {
    const st = barnState[a.id];
    if (!st) return s;
    const _bpsS = st.baseProdStart > 0 ? st.baseProdStart : st.prodStart > 0 ? st.prodStart : 0;
    return s + (_bpsS > 0 ? Math.min(Math.floor((barnNow - _bpsS) / a.prodMs), a.storageMax) * st.owned : 0);
  }, 0);
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3 xl:grid-cols-3">
                {ANIMALS.filter(a => lvl >= a.unlockLevel).map(a => {
                  ...
                  const remaining = st.prodStart > 0 ? Math.max(0, a.prodMs - (barnNow - st.prodStart)) : 0;
                  const pct = st.prodStart > 0 ? Math.min(100, ((barnNow - st.prodStart) / a.prodMs) * 100) : 0;
                  return (
                    <div key={a.id} onClick={() => setSelectedAnimal(a.id)}
                      ...
                      <div className="flex items-center gap-3 mb-2">
                        {/* Obrazek zwierzęcia — 50% większy niż poprzednio (h-10→h-16) */}
                        <AnimalImg id={a.id} icon={a.icon} className="h-16 w-16 text-5xl shrink-0" />
                        ...
                        {(() => {
                          const _bpsOv = st.baseProdStart > 0 ? st.baseProdStart : st.prodStart > 0 ? st.prodStart : 0;
                          const cyclesOv = _bpsOv > 0 ? Math.min(Math.floor((barnNow - _bpsOv) / a.prodMs), a.storageMax) : 0;
                          const itemsOv = cyclesOv * st.owned;
                          return itemsOv > 0 ? <span className="rounded-full bg-green-500/20 border border-green-500/40 px-2 py-0.5 text-[11px] font-black text-green-300 shrink-0">{itemsOv}/{st.owned} {item.icon}</span> : null;
                        })()}
                      </div>
                      {st.owned > 0 && (
                        <>
                          ...
                          <div className="flex justify-end text-[9px] text-[#6b7280]">
